    ...
    <div className="min-h-screen bg-bg-0">
      <header className="sticky top-0 z-10 border-b border-border bg-bg-1/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center gap-4 px-4 py-4">
          <Link href="/chat" className="flex items-center gap-2 text-text-2 hover:text-text-1">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-2 block text-sm font-medium text-text-1">Display Name</label>
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-text-1">Email</label>
...
function PreferencesSettings() {
  const [defaultModel, setDefaultModel] = useState('pico')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-2 block text-sm font-medium text-text-1">Default AI Model</label>
          <select value={defaultModel} onChange={(e) => setDefaultModel(e.target.value)}
            ...
            <option value="pico">Zero Pico (Local WebGPU)</option>
            <option value="nano">Zero Nano (Fast Cloud)</option>
            <option value="prime">Zero Prime (Balanced)</option>
            <option value="apex">Zero Apex (Max Power) — Pro</option>
            <option value="agentic-chad">Agentic Chad (Agent) — Ultra</option>
          </select>
        </div>
        <ToggleSetting title="Auto-save conversations" description="Automatically save chat history" value={autoSave} onChange={setAutoSave} />
        <ToggleSetting title="Smart suggestions" description="Show AI-powered response suggestions" value={suggestions} onChange={setSuggestions} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div>
        ...
        <p className="mt-1 text-sm text-text-2">Customize how Zero AI looks on your screen</p>
      </div>
      <div className="space-y-4">
        <div>
          ...
          <div className="flex gap-3">
            {[
              ...
              <button 
                ...
                className={cn(
                  'flex-1 flex items-center justify-center gap-2 rounded-xl border px-4 py-3.5 text-sm font-semibold transition-all duration-200',
                  theme === t.id
                    ? 'border-zero-300 bg-zero-300/10 text-zero-300 shadow-[0_0_15px_rgba(139,92,246,0.1)]'
                    : 'border-border bg-bg-1 text-text-2 hover:border-border-hover hover:bg-bg-2'
                )}>
                {t.icon}
                {t.label}
              </button>
  ...
  return (
    <div className="space-y-6">
        ...
        {connectors.map((connector) => (
          <div key={connector.id}
            className={cn(
              ...
              connector.status === 'connected' ? 'border-green-500/30 bg-green-500/5' : 'border-border bg-bg-1'
            )}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {connector.capabilities.map((cap) => (
                    <span key={cap} className="rounded bg-bg-2 px-2 py-0.5 text-xs text-text-3">{cap}</span>
                  ))}
                </div>
              </div>
      ...
      <div className="rounded-xl border border-border bg-bg-1 p-4">
        <button onClick={() => setShowMcpServer(!showMcpServer)} className="flex w-full items-center justify-between">
        ...
        {showMcpServer && (
          <div className="mt-4 space-y-3 border-t border-border pt-4">
            <div>
              ...
              <input type="url" value={mcpServerUrl} onChange={(e) => setMcpServerUrl(e.target.value)}
      ...
      <div className="rounded-xl border border-border bg-bg-1 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-sm text-text-2">Export your Zero memories to use with Claude, Cursor, or other MCP-compatible apps.</p>
...
function PrivacySettings() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <ToggleSetting title="Usage analytics" description="Help improve Zero AI with anonymous usage data" value={analytics} onChange={setAnalytics} />
        <div className="rounded-lg border border-red-500/30 bg-red-500/5 p-4">
          <h3 className="mb-2 font-medium text-red-400">Danger Zone</h3>
          <p className="mb-4 text-sm text-text-2">Delete all your data including conversations and memories. This action cannot be undone.</p>
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-border bg-bg-1 p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-text-1">{memoryCount} / {maxMemory === 999999 ? 'Unlimited' : maxMemory}</span>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-bg-3">
            <div className="h-full bg-zero-300" style={{ width: `${Math.min((memoryCount / maxMemory) * 100, 100)}%` }} />
          </div>
        </div>
        <button className="w-full rounded-lg border border-border bg-bg-1 px-4 py-3 text-sm text-text-2 transition-colors hover:border-border-hover hover:text-text-1">View All Memories</button>
        <button className="w-full rounded-lg border border-border bg-bg-1 px-4 py-3 text-sm text-text-2 transition-colors hover:border-border-hover hover:text-text-1">Export for Claude/Cursor (MCP)</button>
        <button className="w-full rounded-lg border border-red-500/30 bg-red-500/5 px-4 py-3 text-sm text-red-400 transition-colors hover:bg-red-500/10">Clear All Memories</button>
      </div>
    </div>
  )
}
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-bg-1 p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="mt-1 flex items-center gap-2"><PlanBadge plan={plan} /></div>
          </div>
          {plan === 'starter' && (
            <Link href="/pricing" className="rounded-lg bg-zero-300 px-4 py-2 text-sm font-medium text-bg-0 transition-colors hover:bg-zero-400">Upgrade</Link>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-bg-1 p-4">
        ...
        <ul className="mt-3 space-y-2 text-sm text-text-2">
          <li className="flex items-center justify-between">
            <span>Zero Pico (Local)</span>
            <span className="font-medium text-emerald-400">Always Free</span>
          </li>
          <li className="flex items-center justify-between">
            <span>Zero Nano</span>
          ...
          <li className="flex items-center justify-between">
            <span>Zero Apex</span>
            <span className="font-medium text-zero-300">{plan === 'pro' || plan === 'ultra' ? 'Included' : 'Pro Plan'}</span>
          </li>
          <li className="flex items-center justify-between">
            <span>Agentic Chad</span>
            <span className="font-medium text-zero-300">{plan === 'ultra' ? 'Included' : 'Ultra Plan'}</span>
          </li>
        </ul>
      </div>
      ...
      {plan !== 'starter' && (
        <div className="space-y-3">
          ...
          <button className="w-full rounded-lg border border-border bg-bg-1 px-4 py-3 text-sm text-text-2 transition-colors hover:border-border-hover hover:text-text-1">Manage Billing</button>
          <button className="w-full rounded-lg border border-red-500/30 bg-red-500/5 px-4 py-3 text-sm text-red-400 transition-colors hover:bg-red-500/10">Cancel Subscription</button>
        </div>
      )}
    </div>
  )
...
function ToggleSetting({ title, description, value, onChange }: { title: string; description: string; value: boolean; onChange: (value: boolean) => void }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-border bg-bg-1 p-4">
      ...
      <button onClick={() => onChange(!value)}
        className={cn('relative h-6 w-11 shrink-0 rounded-full transition-colors', value ? 'bg-zero-300' : 'bg-bg-3')}>
